...
* ~~"Comparative unit" / "comparative population" → "Unit" / "Population" — Documentation settles on plain "Unit" and "Population" as the standard terms (per the Primer), with "Reporting Unit" retained as a named special case of Unit. Codebase currently uses the compound form throughout: the `ComparativeUnit` class (`m04_data_shapes/shapes.py`) and its subclasses, "comparative population" in docstrings, and a stray "comparator population" string in the `app.py` UI caption. All require updating to match — class rename, docstring updates, and the one leftover UI string, which was already inconsistent with the Primer's own stated convention before this decision.~~ Done — `ComparativeUnit` renamed to `Unit` in `m04_data_shapes/shapes.py` (subclasses `NumericSeriesUnit`, `NumericCompositionalUnit`, `CategoricalCompositionalUnit` unchanged in name, now inheriting from `Unit`); "comparative unit"/"comparative population" wording in docstrings updated to plain "unit"/"population"; the stray "comparator population" string in the `app.py` Populations caption corrected to "Population tables define the population."

* ~~"Item table" terminology — "item" is an early, superseded name for what is now called "Unit"/"Reporting unit". Appears in `app.py` (a UI caption and an expander label, "Submissions — item table") — the `m10_project_config/submissions.py` docstring instance no longer applies, since that module has been retired and its one live function now lives in `app.py` directly. Rename throughout code and documentation to **Population table** — decided in preference to "Unit table," since "Population" is the stronger human-facing term and "Unit" reads as weak outside the codebase.~~ Done — renamed to **Population table** throughout: the `app.py` expander label (now "Units — population table"), Glossary Cluster 9, and the Architecture §4 `workfile_config/units.csv` row.

* ~~"Submission" / `submission_id` / `submission_code` terminology — should be renamed to Unit-based terminology throughout ("submission" fits as a description, since a Trust submits data, but Unit is the standardised term going forward). Also note: the current Glossary entry's description of Submission as "a single reporting unit's data record" is not accurate as stated — needs correcting as part of this same pass, not just renamed.~~ Done — scope as landed:
  - Boundary drawn at the API layer: `api_client.py`'s `get_submissions()` and the raw JSON keys it reads (`submissionId`, `submissionCode`, `submissionName`) are unchanged — that's the external contract, and genuinely is "submission" data.
  - Everything from the point of ingestion onward renamed to Unit-based terminology: `unit_id`/`unit_code`/`unit_name` (`ReportContext`, the `Unit` data-shape fields), `WorkfileState.units` (renamed from `.submissions`), `units.csv` (renamed from `submissions.csv`), settings key `selected_unit_id` (renamed from `selected_submission_id`).
  - Fields describing the submission event itself rather than unit identity — `submission_year`, `submission_level`, `submission_service_count` — were left unrenamed, since "submission" remains an accurate description there.
  - Glossary's "Submission / submission ID / submission code" entry corrected and renamed to "Unit / unit ID / unit code" — the old description ("a single reporting unit's data record") was inaccurate; it now describes the identifier fields, not the data record itself.
  - Downstream consistency renames also applied: "Multiple submissions from same org" → "Multiple units from same org" (Feature List); "Multi-submission table expansion" → "Multi-unit table expansion" (Feature List, Functional Spec §11.2).

* ~~"Flag token" terminology — the term is disliked; review for a clearer alternative. Used for template placeholder strings (e.g. `[selected-reporting-unit-name]`) replaced per-unit by `update_text`, distinct from the Running Order's Enabled column. Replacement term to be agreed before executing the rename.~~ Done — renamed to **Text Tag** throughout code (`update_text` docstrings/comments, `app.py` Text tab) and documentation (Glossary Cluster 8, Functional Spec §3.2/§9.2/§11.1/§12, Feature List).

* ~~"Batch" terminology — Run Selected (a single reporting unit), Run Batch (a subset), and Run All (the full population) are all currently grouped under "Batch Processing," but "batch" conventionally implies more than one. Review and either accept the current usage or find clearer terminology, consistent across the tab name, code, and documentation. Replacement term to be agreed before executing the rename.~~ Done — resolved as a split: the Streamlit tab renamed from "Batches"/"Batch Processing" to **Outputs** (short) / **Create Outputs** (long), since a single report or the full population aren't naturally "a batch" to a human reader. `Run Selected`/`Run Batch`/`Run All`, the internal batch-processing concept, and the Feature List "Batch processing" section heading are unchanged — "batch" is the right word for the machine-level processing loop, and "Run Batch" specifically is the one place a human genuinely means "more than one, less than all."
...
* Type coercion at the CSV/WorkfileState boundary is currently ad hoc (fixed once, for `enabled`, and once for `unit_id` — Session 4) rather than systematic. Worth a single normalisation step applied uniformly at every Running Order entry point, or a move to typed dataclasses for Running Order rows instead of raw dicts. `organisation_id` is known to have the same unaddressed inconsistency as `unit_id` did — raw int from the API, string once round-tripped through CSV — currently masked by defensive `str()` wrapping at every comparison site rather than fixed at ingestion.
...
* Text Engine and Batch Controller as dedicated modules — text tag replacement and the batch iteration loop are currently not split into their own module folders. Planned as part of the refactor.
