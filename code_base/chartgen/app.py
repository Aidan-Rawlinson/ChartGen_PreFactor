...
from packages.data_acquisition.fetch import fetch_all
from packages.data_acquisition.api_client import get_token
from packages.chart_engine.cache_reader import list_cached_files, load_shape, load_manifest
from packages.chart_engine.chart_type_map import get_valid_chart_types
from packages.chart_engine.base_charts import render_chart
from packages.local_config.local_config import load_last_username, save_last_username
from packages.workfile_file.workfile_file import (
...
CHART_TYPE_MAP_PATH = os.path.join(
    os.path.dirname(__file__), "packages", "static_config", "chart_type_map.csv"
)
...
def _show_new_workfile_form():
    from packages.data_acquisition.api_client import get_projects, get_submissions, get_organisations
...
with tab_imports:
    ...
    if uploaded_template is not None:
        if st.button("Process Template"):
            from packages.template_reader.template_reader import read_template, update_urls_csv
            from packages.running_order.running_order import generate_from_template
...
with tab_text:
    ...
    from packages.local_config.local_config import build_report_context as _brc_text
...
with tab_running_order:
    ...
    if not ws_ro.running_order_rows:
        ...
        try:
            import io
            import pandas as pd
            from packages.running_order.running_order import (
            ...
            def _row_edit_dialog(sel_idx):
                ...
                if needs_populations:
                    from packages.local_config.local_config import get_peer_group_value_options
    ...
    if not cached_files:
        ...
        if not valid_types:
            ...
            if selected_desc:
                ...
                from packages.local_config.local_config import build_report_context
                from packages.assembly_engine.assembly_engine import build_population_shapes
...
with tab_outputs:
    ...
    from packages.assembly_engine.assembly_engine import run_running_order
    ...
    def _run_for_units(units_to_run: list, run_label: str):
        from packages.assembly_engine.assembly_engine import (
